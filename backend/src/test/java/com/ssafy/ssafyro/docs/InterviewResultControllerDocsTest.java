...
public class InterviewResultControllerDocsTest extends RestDocsSupport {
    ...
    void getBestInterviewResults() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/interview-result/user/best")
    ...
    void getInterviewResultsByUserId() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/interview-result/user")
                                .header("Authorization", "Bearer {JWT Token}")
                                .contentType(MediaType.APPLICATION_JSON)
                )
                .andDo(print())
    ...
    void getInterviewResults() throws Exception {
        ...
        given(interviewResultService.getInterviewResultsBy(any(List.class), any(Pageable.class)))
                .willReturn(response);

        mockMvc.perform(
                        get("/api/v1/interview-result")
                                .queryParam("query", "태그1,태그2,태그3")
                                .queryParam("sort", "evaluationScore,desc")
                                .contentType(MediaType.APPLICATION_JSON)
                )
                .andDo(print())
                .andExpect(status().isOk())
                .andDo(document("get-interview-result",
                                preprocessResponse(prettyPrint()),
                                queryParameters(
                                        parameterWithName("query")
                                                .description("검색어"),
                                        parameterWithName("sort")
                                                .description("createDate: 최신순 정렬, evaluationScore: 점수순 정렬")
                                ),
                                responseFields(
                                        fieldWithPath("success").type(JsonFieldType.BOOLEAN)
                                                .description("성공 여부"),
